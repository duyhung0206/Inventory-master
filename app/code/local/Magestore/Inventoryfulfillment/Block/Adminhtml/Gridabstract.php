 ...
 */
abstract class Magestore_Inventoryfulfillment_Block_Adminhtml_Gridabstract extends Mage_Adminhtml_Block_Widget_Grid {
    ...
    protected function _filterDateCallback($collection, $column) {
        ...
        if (isset($filter['orig_from'])) {
            ...
            $collection->getSelect()->where($field . ' >= \'' . $from . '\'');
        }
        if (isset($filter['orig_to'])) {
            $to = date('Y-m-d H:i:s', strtotime($filter['orig_to']));
            $collection->getSelect()->where($field . ' <= \'' . $to . '\'');
        }
        return $collection;
    }
     ...
     * @return mixed
     */
    public function _filterStoreCallBack($collection, $column)
    {
        $filter = $column->getFilter()->getValue();
        $field = $this->_getRealFieldFromAlias($column->getIndex());
        $collection->addFieldToFilter($field, array('where' => $filter));
        return $collection;
    }
